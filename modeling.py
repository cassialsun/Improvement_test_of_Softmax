...
class LabelSmoothingLoss(_Loss):
    def __init__(self, label_smoothing=0, tgt_vocab_size=0, ignore_index=0, size_average=None, reduce=None, reduction='mean'):
        ...
        smoothing_value = label_smoothing /f (tgt_vocab_size - 2)
...
class BioCls(BertLMPredictionHead):
    def __init__(self,config):
        super().__init__(config=config)
        self.decoder = nn.Linear(config.hidden_size, config.bio_size, bias=False)
        self.bias = nn.Parameter(torch.zeros(config.bio_size))
    ...
    def forward(self, hidden_states):
        ...
        predict_score = self.decoder(hidden_states)
...
class myLoss():
    def __init__(self):
        pass
    def __call__(self,input_, target_,vals):

        '''
        Input dim = [batch_size, sequence length, hidden states]
        vals dim = [batch_size, sequence length, topk]
        target dim = [batch_size, sequence length]
        '''

        if input_.size(0) != target_.size(0):
            raise ValueError('Expected input batch_size ({}) to match target batch_size ({}).'
                             .format(input_.size(0), target_.size(0)))

        #input_= F.softmax(input_, dim=-1)  # shape [num_samples,num_classes]

        dim = input_.dim()
        assert (dim == 3)
        bz,sl = target_.shape[0],target_.shape[1]
        input_ = input_.contiguous().view(-1,input_.shape[-1])
        target_ = target_.contiguous().view(-1,1)
        vals = vals.contiguous().view(-1,vals.shape[-1])


        pos_loss = torch.gather(input_,-1,target_)
        neg_loss = torch.unsqueeze(torch.logsumexp(vals,dim = -1),1)

        lossMatix = neg_loss - pos_loss
        lossMatix = lossMatix.view(bz,sl)
        return lossMatix


        # topk_loss = 0
        # one_hot=torch.zeros_like(probability).scatter_(-1,torch.unsqueeze(target_,dim=-1),1).type(torch.float)
        # loss = - one_hot * probability
        # #topk_loss = loss[:,:,idxs[:,:,]]
        # loss = loss.view(-1,loss.shape[-1])
        # idxs = idxs.view(-1,idxs.shape[-1])
        # a = [v.index_select(dim=-1, index=idxs[i]) for i, v in enumerate(loss)]
        # #b = torch.Tensor(idxs.shape[0],idxs.shape[1])
        # #torch.cat(a,out=b)
        # b = torch.stack(a)
        # topk_loss = b.mean(dim=-1)
        # topk_loss = topk_loss.view(probability.shape[0],probability.shape[1])
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = UnilmModel(config)
        self.cls = BertOnlyMLMHead(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction = 'none')#reduction='none'
        if hasattr(config, 'label_smoothing') and config.label_smoothing:
            self.crit_mask_lm_smoothed = LabelSmoothingLoss(
                config.label_smoothing, config.vocab_size, ignore_index=0, reduction='none')
        else:
        ...
        self.bio_cls = BioCls(config)
        self.K = 10
        self.topK_loss = myLoss()
    def tie_weights(self):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                masked_pos=None, masked_weights=None, num_tokens_a=None, num_tokens_b=None, bio_labels=None,
                lm_bio_labels=None,mode = 'normal'):
        ...
        if mode2 == 'topk':
            new_prediction_scores = prediction_scores_masked
            top_v, top_i = torch.topk(prediction_scores_masked,self.K, dim = 2)

        if mode2 == 'all':
            new_prediction_scores = prediction_scores_masked

        if mode == 'bio':
            bio_scores_masked = self.bio_cls(sequence_output_masked)
        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(
                F.log_softmax(new_prediction_scores.float(), dim=-1), masked_lm_labels)
        else:
            if mode2 == 'topk':
                topk_loss = self.topK_loss(new_prediction_scores, masked_lm_labels, top_v)
                topk_loss = loss_mask_and_normalize(topk_loss.float(), masked_weights)
                return topk_loss

            masked_lm_loss = self.crit_mask_lm(
                new_prediction_scores.transpose(1, 2).float(), masked_lm_labels)
            if mode == 'bio':
                masked_bio_loss = self.crit_mask_lm(
                    bio_scores_masked.transpose(1, 2).float(), lm_bio_labels)

        masked_lm_loss = loss_mask_and_normalize(
            masked_lm_loss.float(), masked_weights)
        if mode == 'bio':
            masked_bio_loss = loss_mask_and_normalize(
                masked_bio_loss.float(), masked_weights)
            masked_lm_loss = masked_lm_loss + masked_bio_loss
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def get_ngram_set(self, x, n):
        """ generating set of ngrams，the return format is :
        {(n-1)-gram: set([the sets of n-th token in ngrams])}
        """
        result = {}
        n = int(n)
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask,mode = 'normal',mode2 = 'topk'):
        ...
        if mode == 'bio':
            while next_pos < output_length:
                ...
                last_hidden = new_encoded_layers[-1][:, -1:, :]
                prediction_scores = self.cls(last_hidden)
                bio_scores = self.bio_cls(last_hidden)
                softmax = torch.nn.Softmax(dim=-1)

                # set up states: if the previous state is 0, current bio is not able to be 2
                if flag != 0:
                    for idx, _ in enumerate(bio_scores):
                        if states[idx] == 0:
                            bio_scores[idx, 0, 2] = -np.inf
                    _, bio_labels = torch.max(bio_scores, dim=-1)
                    bio_labels = bio_labels.squeeze(dim=1)
                else:
                    for idx, _ in enumerate(bio_scores):
                        bio_scores[idx, 0, 2] = -np.inf
                    bio_scores = softmax(bio_scores)
                    _, bio_labels = torch.max(bio_scores, dim=-1)
                    bio_labels = bio_labels.squeeze(dim=1)
                    states = bio_labels.clone().detach().cpu().numpy()

                probas = torch.zeros_like(prediction_scores) - np.inf

                for i, pred_s in enumerate(prediction_scores):
                    # TODO: bio_scores[:,2] = -float('inf')
                    cur_bio_label = int(bio_labels[i])
                    if cur_bio_label < 2:
                        states[i] = cur_bio_label
                    else:
                        states[i] += 1

                    if states[i] > 1 and flag != 0:
                        ngrams = self.get_ngram_set(input_ids[i].cpu().numpy().tolist(), states[i])
                        tensor_output = torch.stack([item[i] for item in output_ids])
                        # prefix_ = tuple(tensor_output[i, 1 - int(states[i]):])
                        temps = tensor_output[1 - int(states[i]):]
                        prefix = []
                        for temp in temps:
                            prefix.append(int(temp))
                        prefix = tuple(prefix)

                        if prefix in ngrams:  # if n-gram is eligible
                            candidates = ngrams[prefix]
                        else:  # if not, go back to 1-gram
                            ngrams = self.get_ngram_set(input_ids[i].cpu().numpy().tolist(), 1)
                            candidates = ngrams[tuple()]
                            states[i] = 1

                        candidates = list(candidates)
                        # TODO: candidates += no_mask function
                        probas[i, 0, candidates] = prediction_scores[i, 0, candidates]


                    else:
                        probas[i] = prediction_scores[i]

                prediction_scores = softmax(probas)
                _, max_ids = torch.max(prediction_scores, dim=-1)
                output_ids.append(max_ids)

                if prev_embedding is None:
                    prev_embedding = new_embedding[:, :-1, :]  # -1 : The last one is [SEP]
                else:
                    prev_embedding = torch.cat(
                        (prev_embedding, new_embedding[:, :-1, :]), dim=1)
                if prev_encoded_layers is None:
